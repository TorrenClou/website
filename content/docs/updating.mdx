...
older image may not start against the newer schema — and if it does start, it is running
against a shape it does not know about. Restore the pre-upgrade dump as well. That is the
reason to take one.

In practice: rolling back one release is usually fine, rolling back across a release that
changed the schema is not, and the dump is what makes the difference.
...
```bash
docker inspect ghcr.io/torrenclou/torrentclou:latest \
