...
description: Monitor your TorrenClou instance with built-in Grafana dashboards.
---

## Overview

Grafana, Prometheus, and Loki are **bundled inside the TorrenClou Docker container** — no extra setup needed. After installation, monitoring is available immediately:
...
TorrenClou exposes a `/metrics` endpoint on the API port:
...
With `OBSERVABILITY_LOKI_URL` set, TorrenClou pushes structured logs to the built-in Loki instance. It is unset by default, and the bundled Grafana and Prometheus work without it.
...
With `OBSERVABILITY_ENABLE_TRACING=true` (enabled by default), TorrenClou exports distributed traces.
...
To ship logs and traces to an external backend, pass these as environment variables:
...
These are read at start, so restart the container after changing them:
